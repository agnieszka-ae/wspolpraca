...
const SidebarStyles = `
  ...
  .ae-sidebar-logo { width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; margin-bottom: 8px; }
  .ae-sidebar-divider { width: 36px; height: 1px; background: rgba(20,20,20,0.10); margin: 4px 0; }
  .ae-nav-icon {
  ...
  .ae-nav-icon:hover { background: rgba(116,72,208,0.07); color: rgba(5,5,5,0.75); }
  .ae-nav-icon.active { background: #F5F1FE; color: #7448D0; }
    ...
    <React.Fragment>
      ...
      <div className="ae-sidebar">
        <div className="ae-sidebar-logo">
          <svg width="28" height="23" viewBox="0 0 36 30" fill="none">
        ...
        {main.map(item => (
          <div key={item.id} className={`ae-nav-icon${active === item.id ? ' active' : ''}`} onClick={() => onNavigate(item.id)} title={item.label}>
            <item.icon />
          </div>
        ))}
